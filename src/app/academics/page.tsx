...
export async function generateMetadata({ searchParams }: PageProps): Promise<Metadata> {
  ...
  return buildMetadata({ schoolName: data?.name, title: 'Academics', description: data?.academics.description })
}
...
export default async function AcademicsPage({ searchParams }: PageProps) {
  ...
  if (data.config.template_id === 'template-a') return <TemplateA data={data} page="academics" />
  if (data.config.template_id === 'template-b') return <TemplateB data={data} page="academics" />
  return notFound()
}
